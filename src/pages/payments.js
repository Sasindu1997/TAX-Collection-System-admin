...
import { CustomersTable } from 'src/sections/payments/customers-table';
import { CustomersSearch } from 'src/sections/payments/customers-search';
...
const Page = () => {
  ...
  useEffect(() => {
    ...
    setOpenBackDrop(true)
    cusId && SDK.PaymentType.getById(cusId)
    .then((res) => {
      console.log("RES: ", res);
      setCustomerData(res?.data);
      setStatus(res?.data?.payment_mode);
      setCollectorId(res?.data?.collector);
      reset(res?.data);
      setOpenBackDrop(false)
    })
    .catch((error) => {
  ...
  const handleDelete = () => {
    cusId && SDK.PaymentType.deletebyId(cusId)
    .then((res) => {
      console.log("RES: ", res);
      setOpenDelete(false);
  ...
  const handleChangeCollectorId = (event) => {
  ...
  const handleSubmitUpdate = (values) => {
    ...
    cusId && SDK.PaymentType.updatePatch(cusId, obj)
    .then((res) => {
      console.log("RES: ", res);
      setOpenUpdate(false);
      setSnackSeverity('success');
            ...
            <Dialog open={open} onClose={handleClose}>
              ...
              <DialogContent>
                ...
                <Select
                  ...
                  fullWidth
                  variant="standard"
                  onChange={handleChangeCollectorId}
                >
                  <MenuItem value={'1bb5d9d2-dee3-4b98-82b1-90239c173113'}>1bb5d9d2-dee3-4b98-82b1-90239c173113</MenuItem>
